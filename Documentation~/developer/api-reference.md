...
### `emiteat.NexUI.Studio.Editor.NexUIDesignerWindow`

메인 Unity Editor 창입니다. (네임스페이스는 `emiteat.NexUI.Studio.Editor` 이며 `.Core` 하위가 아닙니다.)
...
### `emiteat.NexUI.Studio.Editor.NexUIDesignerContext`

현재 스튜디오 세션의 공유 상태입니다.
...
### `emiteat.NexUI.Studio.Editor.Backend.INexUIDesignerBackend`

UI 구현을 스튜디오에 연결하기 위한 백엔드 계약입니다.
...
### `emiteat.NexUI.Studio.Editor.Serialization.IDesignerAssetSerializer`
...
- `UGUIAssetSerializer` — 프리팹 기반 저장. `PrefabUtility.LoadPrefabContents → SaveAsPrefabAsset → UnloadPrefabContents` 패턴으로 RectTransform/텍스트/틴트/Button 등 스튜디오 소유 데이터를 프리팹에 반영합니다. Element는 `stableId`로 Prefab Object에 연결됩니다.
...
### `emiteat.NexUI.Studio.DesignerMetadataAsset`

스튜디오 전용 화면 메타데이터의 루트 에셋입니다. (런타임 안전 메타데이터의 실제 네임스페이스는 `emiteat.NexUI.Studio` 이며, 파일 위치는 `Runtime/Metadata/` 입니다. 이 어셈블리는 `UnityEditor`를 참조하지 않습니다.)
...
### `emiteat.NexUI.Studio.Editor.Components.Definitions`
...
### `emiteat.NexUI.Studio.Editor.Properties.DesignerPropertyApplier`
...
### `emiteat.NexUI.Studio.Editor.UI.Panels.DesignerAssetDropResolver`
...
### `emiteat.NexUI.Studio.Editor.Validation.DesignerValidationService`
