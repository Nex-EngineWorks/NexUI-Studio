...
| 데이터 | 주 수정자 | Studio 동작 | Runtime 사용 |
| --- | --- | --- | --- |
| `UIScreenDefinition` | 사용자/설정 도구 | 연결 정보 저장 | 예 |
| `DesignerMetadataAsset` | Studio | Element, Binding, Motion 참조 저장 | Runtime-safe 형식이나 주 용도는 제작 데이터 |
...
Studio Metadata와 Screen Definition을 저장하고 Backend별 Serializer를 실행합니다. uGUI는 지원되는 Prefab 값을 반영합니다. UI Toolkit은 수동 UXML을 보존하는 companion-save 방식입니다.

## Generate와 Generated Marker

Generator는 Metadata 옆에 Screen 이름을 기준으로 `.g.uxml`, `.g.uss`를 만듭니다. 파일에는 Studio 생성물임을 나타내는 Marker가 들어갑니다. Marker가 없는 기존 파일은 사용자 소유로 간주하여 덮어쓰기를 거부합니다.
...
`DesignerPublishManifest`는 Screen별 마지막 UXML/USS Hash를 저장합니다. 현재 Studio 출력, 디스크 파일, 마지막 Publish Base를 비교하여 `New`, `InSync`, `DesignerChanged`, `BackendChanged`, `Conflict`를 판정합니다. 자동 Merge는 하지 않습니다.
...
- **Studio 사용**: 생성 결과를 기준으로 파일을 다시 씁니다.
