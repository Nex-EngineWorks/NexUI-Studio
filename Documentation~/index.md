...
> **처음이라면 [전체 개요](overview.md)** — 프로젝트를 만든 이유, 런타임과 Studio의 전체 기능, 컴포넌트 라이브러리, 백엔드 출력, 현재 상태, 그리고 모든 문서로 가는 지도를 한 문서에 정리했습니다.

## 처음 사용하는 사용자

- [설치](getting-started/installation.md) — Core와 Studio 패키지를 올바른 순서로 설치합니다.
...
- [Studio 창](user-guide/designer-window.md) — Toolbar, Sidebar, Canvas, Inspector와 Bottom Drawer의 역할을 설명합니다.
...
- [Assets 패널](user-guide/assets-panel.md) — Studio 안에서 Sprite/Font/Component를 찾아 캔버스로 드래그합니다.
...
- [Asset Ownership](reference/asset-ownership.md) — 수동 파일, Studio 파일과 생성 파일의 책임을 구분합니다.
